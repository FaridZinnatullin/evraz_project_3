...
def service_booking(booking_repo, books_repo, publisher):
    return services.BookingManager(booking_repo=booking_repo, books_repo=books_repo, publisher=publisher)
...
def test_get_by_book_id(service_booking):
    booking = service_booking.booking_repo.get_by_book_id(book_id=data_booking['book_id'])
    assert asdict(booking) == data_booking


def test_get_by_user_id(service_booking):
    booking = service_booking.booking_repo.get_by_user_id(user_id=data_booking['user_id'])
    assert asdict(booking) == data_booking


def test_get_active_booking(service_booking):
    booking = service_booking.get_active_booking(user_id=data_booking['user_id'])
    assert asdict(booking) == data_booking
...
def test_check_book_available(service_booking, booking_repo):
    ...
    result = service_booking.check_book_available(book_id=data_booking['book_id'], user_id=data_booking['user_id'])
    assert result == True


def test_check_book_available_user_already_have_booking(service_booking, booking_repo):
    overdude_booking = booking_repo.get_overdude_booking()
    booking_repo.get_by_book_id.return_value = overdude_booking

    with pytest.raises(errors.UserAlreadyHaveBooking):
        service_booking.check_book_available(book_id=data_booking['book_id'], user_id=data_booking['user_id'])


def test_check_book_available_book_already_booked(service_booking, booking_repo):
    overdude_booking = booking_repo.get_overdude_booking()
    booking_repo.get_by_user_id.return_value = overdude_booking
...
def test_check_permission_positive(service_booking):
    assert service_booking.check_permission(booking_id=data_booking['id'], user_id=data_booking['user_id']) == True


def test_check_permission_negative(service_booking):
    booking_id = 3
    user_id = 3
    assert service_booking.check_permission(user_id=user_id, booking_id=booking_id) == None
...
def test_booking_book(service_booking, booking_repo):
    ...
    service_booking.booking_book(book_id=data_booking['book_id'],
...
def test_delete_booking(service_booking, booking_repo):
    service_booking.delete_booking(booking_id=data_booking['id'],
                                   user_id=data_booking['user_id'])
    service_booking.booking_repo.add_instance.assert_called_once()
...
def test_redeem_booking(service_booking, booking_repo):
    service_booking.redeem_booking(booking_id=data_booking['id'],
                                   user_id=data_booking['user_id'])
...
def test_redeem_booking_no_permission(service_booking, booking_repo):
    with pytest.raises(errors.NoPermission):
        service_booking.redeem_booking(booking_id=data_booking_2['id'],
                                       user_id=data_booking_2['user_id'])
...
def test_get_all_users_booking(service_booking):
    bookings = service_booking.get_all_users_booking(user_id=data_booking['user_id'])

    assert [asdict(booking) for booking in bookings] == [data_booking, data_booking_1]
